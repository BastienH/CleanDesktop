...
{"images" : [".jpg", ".png"]} """
    ...
    mkdirs(sorting_dict)
    ##Rework: create a virtual Desktop as a Dictionary, only mkdir for keys (folders) that contain a value (file).
    #           virtual_desktop_dict = {}

    for src_full_path in src_full_paths: #Input files
        
        src_full_path_lower = src_full_path.lower()
        file_name = basename(src_full_path)
        
        for destination_dir, extensions_list in sorting_dict.items(): #Input sorting pattern
            
            assert type(extensions_list) == list, f"Extensions should be presented in a list [], current value is {extension}"

            dst_dir = pjoin(DESKTOP, destination_dir)
            dst_full_path = pjoin(DESKTOP, destination_dir, file_name)
